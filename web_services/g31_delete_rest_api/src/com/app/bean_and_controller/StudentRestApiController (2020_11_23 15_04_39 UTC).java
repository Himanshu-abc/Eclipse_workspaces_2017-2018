...
public class StudentRestApiController {
	...
	public ResponseEntity<Boolean> updateStudent(@PathVariable("name") String name,@RequestBody Student s) {
		...
		return new ResponseEntity<Boolean>(true,ht,HttpStatus.OK);
	}
	
	// creating or posting  a record
	@RequestMapping(value="/Students",method=RequestMethod.POST,consumes=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Boolean> postStudent(@RequestBody Student s) {
		
		System.out.println(s.getName());
		System.out.println("StudentName : "+s.getName()+" StudentHobby : "+s.getHobby());
		
		// insert new record into database
		
		HttpHeaders ht=new HttpHeaders();
		ht.add("Location", ServletUriComponentsBuilder.fromCurrentRequest().path("/{name}").
				buildAndExpand(s.getName()).toUri().toString());
		 
		return new ResponseEntity<Boolean>(true, ht ,HttpStatus.CREATED);
	}
